...
export default function DiagnoseModal({ isOpen, onClose, serviceName }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1e293b] border border-[#334155] rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#334155] bg-[#0f172a]">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <span className="text-[#3b82f6]">✦</span> AI Diagnosis: {serviceName}
          </h2>
        ...
        <div className="p-6 overflow-y-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-[#94a3b8] animate-pulse">AI is analyzing telemetry and logs...</p>
            </div>
          ) : error ? (
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[#64748b] text-[12px]">Confidence: {result.confidence || '95%'}</span>
              </div>

              <div>
                <h3 className="text-[11px] font-bold text-[#64748b] uppercase tracking-wider mb-2">Root Cause</h3>
                <div className="bg-[#0f172a] border border-[#334155] rounded-lg p-4 text-[#cbd5e1] text-[14px] leading-relaxed">
                  {result.root_cause || result.simple_explanation || "Unable to determine exact root cause."}
                </div>
              </div>

              <div>
                <h3 className="text-[11px] font-bold text-[#64748b] uppercase tracking-wider mb-2">Recommended Fix</h3>
                <div className="bg-[#0f172a] border border-[#334155] rounded-lg p-4 text-[#cbd5e1] text-[14px] leading-relaxed">
        ...
        <div className="px-6 py-4 border-t border-[#334155] bg-[#0f172a] flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-[13px] font-bold text-[#94a3b8] hover:text-white transition-colors">
            Cancel
          </button>
          <button 
