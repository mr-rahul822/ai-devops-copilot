...
export default function ServiceHealthCard({ serviceName, cpu = 0, ram = 0, disk = 0 }) {
  ...
  const handleDiagnose = () => {
    // Navigate to AI Insights page with the service pre-selected and auto-run enabled
    navigate(`/insights?service=${encodeURIComponent(serviceName)}&autorun=true`)
  }
    ...
    <div className="bg-white dark:bg-[#1e293b] border border-[#e2e8f0] dark:border-[#334155] rounded-xl p-4 shadow-sm flex flex-col justify-between">
      ...
      <div className="grid grid-cols-2 gap-2 mt-auto">
        ...
        <button 
          ...
          className="w-full py-1.5 rounded border border-[#e2e8f0] dark:border-[#334155] text-[#64748b] dark:text-[#cbd5e1] text-[11px] font-bold uppercase tracking-wider hover:bg-gray-100 dark:hover:bg-[#334155]/50 transition-colors text-center"
        >
          View Logs
        </button>
      </div>
